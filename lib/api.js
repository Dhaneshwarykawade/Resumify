...
const genAI = new GoogleGenerativeAI("AIzaSyCBnq13OQ9DL1wlSuJVAQeFYEHsNpiXgWY");
...
//  export const analyzeResumeFileBased = async (data) => {
//     try {
//       if (!data.file) return{ error: "No file uploaded" };
...
export const getLanguages = async (data) => {
  try {
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
...
export const translateLabels = async (data) => {
  ...
  try {
    ...
    return { error: "Failed to fetch translated labels" };
  }
};
...
export const translateResumeData = async (data) => {
  try {
    const { resumeData, targetLang } = data;
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
    ...
    return { error: "Failed to fetch translated resume" };
  }
};
